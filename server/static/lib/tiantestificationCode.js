...
function parse_params() {
    let param_str = location.search;
        if (param_str.indexOf("?") === -1) return alert('需要参数')
        param_str = param_str.substr(1);
        var params_items = param_str.split('&');
        var params = {};
        console.log('params_items', params_items)
        for ( var i in params_items) {
            if (param_str.indexOf("=") === -1) continue
            var item = params_items[i].split('=');
            console.log('item', item)
            params[item[0]] = item[1]
        }
        console.log('params', params)
        if (!params['type']) return alert('需要type参数')
        return params
}
...
function requestImages(refresh=false,track=[],ctx) {
    return new Promise(function(resolve, reject){
        ...
        console.log('params', params)
        $.post("http://tiantest.natapp1.cc/collect", "trace=" + JSON.stringify(track) + '&type=' + params['type']);
